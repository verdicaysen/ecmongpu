...
int test() {
  ...
  pthread_mutex_init(&phelper.mutex, NULL);
	...
	//mpz_primorial_ui(gmp_scalar, 8192);
	...
	char nafpath[1000];
	if(readlink("/proc/self/exe", nafpath, 1000) == -1){
		TEST_FAILURE;
	}
	dirname(nafpath);
	strcat(nafpath, "/res/trpl-batch-4096");
  
  FILE *naffile = fopen(nafpath, "rb");
  int scalar_naf_digits = 0;
  if (naffile) {
      scalar_naf_digits = fread(h_scalar_naf, 1, naf_size, naffile);
      printf("read %d naf bytes", scalar_naf_digits);
  } else {
    printf("error (%d) opening naf file: %s\n", errno, nafpath);
  }
